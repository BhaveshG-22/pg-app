...
const presets = [
  ...
  {
    title: "3D Logo on Sportswear",
    description: "Convert a logo into a 3D embroidered patch on jersey fabric.",
    ...
    category: "Transform",
    provider: "OPENAI",
    owner: "ChatGPT for Designers"
  },
  {
    title: "Photorealistic Selfie Poster",
  ...
  {
    title: "Nostalgic Train Portrait",
...
async function resetPresets() {
  try {
    console.log('🗑️  Deleting existing presets...');
    
    // Delete all existing presets
    const deleted = await prisma.preset.deleteMany({});
    console.log(`❌ Deleted ${deleted.count} existing presets`);
    
    console.log('\n🚀 Adding new presets with clean titles...');
    ...
    for (const preset of presets) {
      try {
        ...
        console.log(`✅ Created "${preset.title}" (owner: ${preset.owner})`);
      } catch (error) {
        console.error(`❌ Failed to create "${preset.title}":`, error.message);
      }
    }

    console.log('\n🎉 Finished resetting presets!');
